{
    function myScript(thisObj) {
        function myScript_buildUI(thisObj) {
            var myWindow = (thisObj instanceof Panel) ? thisObj : new Window("palette", "A_Text", undefined, { resizeable: true, closeButton: false });
            ...
            function moveAnchorPoint(numeric, trufals) {
                ...
                for (i = 0; i < thisAnchor.length; i++) {
                    ...
                    if (thisAnchor[i].anchorPoint.isTimeVarying) { //jika layer itu bergerak
                        ...
                        posExpression = false;
                        if (thisAnchor[i].position.dimensionsSeparated != true) {
                            thisAnchor[i].position.expressionEnabled = false;
                            posExpression = true;
                        } else {
                            thisAnchor[i].transform("ADBE Position_0").expressionEnabled = false;
                            thisAnchor[i].transform("ADBE Position_1").expressionEnabled = false;
                            thisAnchor[i].transform("ADBE Position_2").expressionEnabled = false;
                            posExpression = true;
                        }
                        ...
                        if (thisAnchor[i].position.isTimeVarying) {
                            for (keyPos = 1; keyPos <= thisAnchor[i].position.numKeys; keyPos++) {
                                ...
                                if (thisAnchor[i].position.dimensionsSeparated != true) {
                                    ...
                                    if (!thisAnchor[i].transform("ADBE Position_0").isTimeVarying) {
                                        thisAnchor[i].transform("ADBE Position_1").setValueAtKey(keyPos, currentPos[1]);
                                    } else if (!thisAnchor[i].transform("ADBE Position_0").isTimeVarying) {
                                        thisAnchor[i].transform("ADBE Position_0").setValueAtKey(keyPos, currentPos[0]);
                                    } else {
                                        thisAnchor[i].transform("ADBE Position_0").setValueAtKey(keyPos, currentPos[0]);
                                        thisAnchor[i].transform("ADBE Position_1").setValueAtKey(keyPos, currentPos[1]);
                                    }
                            ...
                            if (thisAnchor[i].position.dimensionsSeparated != true) {
                                ...
                                thisAnchor[i].transform("ADBE Position_0").setValue([curPos[0] + xMove]);
                                thisAnchor[i].transform("ADBE Position_1").setValue([curPos[1] + yMove]);
                            }
                        }
                        ...
                        if (posExpression) {
                            if (thisAnchor[i].position.dimensionsSeparated != true) {
                                thisAnchor[i].position.expressionEnabled = true;
                                posExpression = false;
                            } else {
                                thisAnchor[i].transform("ADBE Position_0").expressionEnabled = true;
                                thisAnchor[i].transform("ADBE Position_1").expressionEnabled = true;
                                thisAnchor[i].transform("ADBE Position_2").expressionEnabled = true;
                                posExpression = false;
                            }
            ...
            executeButtons.onClick = function () {
                if (app.project.activeItem == null) {
                    alert("DONT HAVE COMP!");
                    return false;
                }
                if (app.project.activeItem.selectedLayers == null) {
                    alert("DONT Selected Layer!");
                    return false; //have selected layer or not?
                }
                if (app.project.activeItem.selectedLayers.length < 1) {
                    alert("Select too Lower!!");
                    return false; //have selected layer or not?
                }
                ...
                expressionFile = File(folder.path.toString() + "/Aw_Tools/" + dropDownExpressions.selection.toString());
